...
namespace API.Controllers
{
  ...
  public class MeterReadingController : ControllerBase
  {
    ...
    [HttpPost("importCurrentReadings")]
    public async Task<IActionResult> ImportCurrentReadings(IFormFile file, [FromQuery] Months month, [FromQuery] int year)
    {
      if (file == null) return BadRequest("الملف مفقود");

      using var stream = file.OpenReadStream();
      var command = new ImportCurrentReadingsCommand(stream, month, year);
