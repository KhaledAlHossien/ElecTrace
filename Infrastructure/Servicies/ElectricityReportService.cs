    ...
    public byte[] GenerateElectricityExcelReport(IEnumerable<ElectricityReportResponseDto> reportData, string title)
    {
      ...
      using (var workbook = new XLWorkbook())
      {
        ...
        foreach (var item in reportData)
        {
          ...
          else if (item.IsRotated)
          {
            dataRange.Style.Fill.SetBackgroundColor(XLColor.FromHtml("#ffeeba")); // أصفر أغمق قليلاً
            dataRange.Style.Font.FontColor = XLColor.FromHtml("#8a6d3b");        // بني متوسط
          }
          else
          {
